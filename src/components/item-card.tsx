...
export default function ItemCard({ item, priority }: ItemCardProps) {
  ...
  return (
    <Link href={`/items/${item.id}`} className="block">
      <Card className="group relative flex flex-col overflow-hidden border-none bg-card/40 p-0 gap-0 backdrop-blur-sm transition-all hover:shadow-2xl hover:shadow-primary/20 hover:-translate-y-1">
        {/* Image Container */}
        <div className="relative h-[240px] sm:h-[300px] w-full overflow-hidden">
          ...
          <div className="absolute top-3 right-3">
            <Badge className="bg-primary/90 text-primary-foreground border-none px-2 py-1 text-[10px] font-bold backdrop-blur-md shadow-lg">
